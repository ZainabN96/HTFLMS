$(document).ready(function () {
    ...
    loadTrainerCourses(shouldRestoreCourse);
...
function getMessage(xhr) {
    ...
    return err?.errorMessage || err?.innerError || err?.title || err?.message || xhr.responseText;
}
...
function updateCreateLinks(courseId) {
    $('#addModuleBtn').attr('href', '/Trainer/ModulesLessons/CreateModule?courseId=' + courseId);
    $('#addLessonBtn').attr('href', '/Trainer/ModulesLessons/CreateLesson?courseId=' + courseId);
    $('#addQuizBtn').attr('href', '/Trainer/ModulesLessons/CreateQuiz?courseId=' + courseId);
}
...
function updateModuleBasedLinks(courseId, moduleId) {
    if (isValidId(moduleId)) {
        $('#addLessonBtn').attr(
            'href',
            '/Trainer/ModulesLessons/CreateLesson?courseId=' + courseId + '&moduleId=' + moduleId
        );

        $('#addQuizBtn').attr(
            'href',
            '/Trainer/ModulesLessons/CreateQuiz?courseId=' + courseId + '&moduleId=' + moduleId
        );
...
function filterModuleContent(moduleId) {
    if (isValidId(moduleId)) {
        ...
        $('.trainer-lessons-table-row[data-lesson-module-id]').hide();
        $('.trainer-lessons-table-row[data-lesson-module-id="' + moduleId + '"]').show();

        $('.trainer-quiz-table-row[data-quiz-module-id]').hide();
        $('.trainer-quiz-table-row[data-quiz-module-id="' + moduleId + '"]').show();

        return;
    }

    $('.trainer-lessons-table-row[data-lesson-module-id]').show();
    $('.trainer-quiz-table-row[data-quiz-module-id]').show();
}
...
function updateAccessToggle(config) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            storeSelectedCourse(selectedCourseId);
            storeSelectedModule(selectedModuleId);
...
$(document).on('click', '#confirmDeleteBtn', function (e) {
    ...
    $.ajax({
        ...
        error: function (xhr) {
            ...
            var message = getMessage(xhr);
            if (message) {
                setEmptyState(deleteItem.container, message);
            }
        }
    });
});
...
function loadTrainerCourses(shouldRestoreCourse) {
    $.ajax({
        url: '/api/Course',
        type: 'GET',
        ...
        success: function (courses) {
            ...
            if (!courses || courses.length === 0) {
                setEmptyState('#modulesContainer', 'No courses found for this trainer.');
                return;
            }
...
function buildModuleRow(module) {
    ...
    return `
        <div class="dashboard-table-row trainer-modules-table-row module-row"
             data-module-id="${moduleId}">

            <div title="${moduleTitle} - ${moduleDescription}">
                <div class="courses-title" title="${moduleTitle}">
                    ${moduleTitle}
                </div>

                <div class="courses-sub-text" title="${moduleDescription}">
                    ${moduleDescription}
                </div>
            </div>

            <div title="${lessonsCount} Lessons">
            ...
            <div title="${moduleIsActive ? 'Published' : 'Draft'}">
                ${getStatusBadge(moduleIsActive)}
            </div>
            ...
            <div title="${moduleDisplayOrder}">
                ${moduleDisplayOrder}
            </div>

            <div class="trainer-courses-actions">
                <a href="/Trainer/ModulesLessons/EditModule?courseId=${moduleCourseId}&moduleId=${moduleId}"
...
function buildLessonRow(lesson, moduleId, courseId, moduleTitle) {
    ...
    return `
        <div class="dashboard-table-row trainer-lessons-table-row"
             data-lesson-id="${lessonId}"
             data-lesson-module-id="${moduleId}">

            <div title="${lessonTitle} - ${lessonDescription}">
                <div class="courses-title" title="${lessonTitle}">
                    ${lessonTitle}
                </div>

                <div class="courses-sub-text" title="${lessonDescription}">
                    ${lessonDescription}
                </div>
            </div>

            <div title="${moduleTitle}">
                ${moduleTitle}
            </div>

            <div title="Lesson">
                <span class="trainer-course-chip">Lesson</span>
            </div>

            <div title="${lessonIsActive ? 'Published' : 'Draft'}">
                ${getStatusBadge(lessonIsActive)}
            </div>

            <div title="${lessonDisplayOrder}">
                ${lessonDisplayOrder}
            </div>

            <div class="trainer-courses-actions">
                <a href="/Trainer/ModulesLessons/EditLesson?courseId=${courseId}&moduleId=${moduleId}&lessonId=${lessonId}"
                   class="dashboard-btn dashboard-btn-outline trainer-course-action-btn"
...
function buildQuizRow(quiz, moduleId, courseId, moduleTitle) {
    ...
    return `
        <div class="dashboard-table-row trainer-quiz-table-row"
             data-quiz-id="${quizId}"
             data-quiz-module-id="${moduleId}">

            <div title="${quizTitle} - ${quizInstructions}">
                <div class="courses-title" title="${quizTitle}">
                    ${quizTitle}
                </div>

                <div class="courses-sub-text" title="${quizInstructions}">
                    ${quizInstructions}
                </div>
            </div>
            ...
            <div title="${questionsCount}">
                ${questionsCount}
            </div>

            <div title="${attemptsAllowed}">
                ${attemptsAllowed}
            </div>

            <div title="${quizIsActive ? 'Published' : 'Draft'}">
                ${getStatusBadge(quizIsActive)}
            </div>
            ...
            <div class="trainer-courses-actions">
                <a href="/Trainer/ModulesLessons/EditQuiz?courseId=${courseId}&moduleId=${moduleId}&quizId=${quizId}"
                   class="dashboard-btn dashboard-btn-outline trainer-course-action-btn"
                   onclick="storeSelectedCourse('${courseId}'); storeSelectedModule('${moduleId}');">
                    Edit
                </a>
...
function highlightSelectedModule(moduleId) {
    ...
    $('.module-row[data-module-id="' + moduleId + '"]')
        .addClass('selected-module-row');
}
